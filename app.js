...
window.add=()=>{
  ...
  const permissionSource=$('permissionSource').value.trim();
  const permissionAt=$('permissionAt').value;
  const l=normalize({first_name:$('name').value.trim(),last_name:'',role:$('role').value.trim(),company:$('company').value.trim(),industry:$('industry').value.trim(),email:$('email').value.trim(),linkedin_url:$('linkedin').value.trim(),source:$('source').value.trim(),permission,permission_source:permissionSource,permission_at:permissionAt,followup_date:'',opted_out:false,opted_out_at:''});
  if(!l.first_name)return alert('اكتب الاسم بالكامل');
  if(!l.company)return alert('اكتب اسم الشركة');
  if(!l.role)return alert('اختر المنصب');
  if(!l.source)return alert('حدد كيف حصلنا على بيانات العميل');
  if(permission==='YES'&&!permissionSource)return alert('حدد مصدر الإذن بالتواصل');
  if(permission==='YES'&&!permissionAt)return alert('حدد تاريخ الإذن');
  if(permission==='NO'&&permissionSource)l.permission_source=permissionSource;
  if(permission==='NO'&&!permissionSource)l.permission_source='لم يتم الحصول على إذن';
...
window.setPermission=id=>{const i=leads.findIndex(x=>x.id===id);if(i<0)return;const current=leads[i].permission||'';const v=prompt('حالة الإذن بالتواصل: اكتب YES للموافقة أو NO لعدم وجود إذن أو CANCEL لطلب عدم التواصل',current);if(v===null)return;const p=v.trim().toUpperCase();if(p==='CANCEL'){const now=new Date().toISOString();leads[i]={...leads[i],permission:'NO',permission_source:'طلب عدم التواصل',permission_at:now,opted_out:true,opted_out_at:now,status:leads[i].status==='Email Ready'?'Qualified':leads[i].status};persist();render();return}if(!['YES','NO'].includes(p))return alert('القيمة يجب أن تكون YES أو NO أو CANCEL');const source=prompt(p==='YES'?'مصدر الإذن':'سبب عدم وجود الإذن',leads[i].permission_source||'');if(source===null)return;const now=new Date().toISOString();leads[i]={...leads[i],permission:p,permission_source:source.trim()||(p==='NO'?'لم يتم الحصول على إذن':''),permission_at:now,opted_out:p==='NO'&&source.trim()==='طلب عدم التواصل',opted_out_at:p==='NO'&&source.trim()==='طلب عدم التواصل'?now:'',status:p==='NO'&&leads[i].status==='Email Ready'?'Qualified':leads[i].status};persist();render()};
...
window.sendSelected=async()=>{const ids=[...document.querySelectorAll('.lead-check:checked')].map(x=>x.value);if(!ids.length)return alert('حدد العملاء المحتملين للإرسال');if(ids.length>5)return alert('الحد الأقصى 5 عملاء محتملين في الدفعة');const selected=ids.map(id=>leads.find(l=>l.id===id)).filter(Boolean);const bad=selected.filter(l=>!canSend(l));if(bad.length)return alert('كل عميل محتمل يجب أن يحتوي بريدًا + مسودة جاهزة للإرسال + ألا يكون قد طلب عدم التواصل + ألا يكون قد أُرسل له سابقًا');if(!getSendEndpoint())return alert('أعد إعداد رابط الإرسال أولًا');if(!confirm(`تأكيد إرسال ${selected.length} رسائل؟`))return;let ok=0,errors=[];for(const original of selected){const i=leads.findIndex(x=>x.id===original.id);try{leads[i]=beginSend(leads[i]);persist();const data=await sendLead(leads[i]);leads[i]=markSent(leads[i],data.message_id);ok++}catch(e){leads[i]={...leads[i],send_lock:false};errors.push(`${original.company}: ${e.message}`)}}persist();render();alert(`تم الإرسال: ${ok}/${selected.length}${errors.length?'\n\nأخطاء:\n'+errors.join('\n'):''}`)};
window.sendNextBatch=()=>{const batch=nextSendBatch(leads);if(!batch.length)return alert('لا توجد رسائل جاهزة للإرسال');batch.forEach(l=>{const el=document.querySelector(`.lead-check[value="${l.id}"]`);if(el)el.checked=true});alert(`تم تحديد أول ${batch.length} عملاء محتملين جاهزين للإرسال. راجعهم ثم اضغط إرسال المحدد`)};
...
function leadCard(l){const permissionLabel=l.opted_out?'طلب عدم التواصل':l.permission==='YES'?'لديه إذن بالتواصل':l.permission==='NO'?'لا يوجد إذن بالتواصل':'غير محدد';return `<article class="card"><label><input class="lead-check" type="checkbox" value="${l.id}" ${canSend(l)?'':'disabled'}> جاهز للإرسال</label><div><b>${l.first_name||''}</b> — ${l.role||'بدون منصب'}</div><b>${l.company}</b> <span class="muted">${l.industry||''}</span><div>${l.email||'بدون بريد'}</div><div class="muted">${l.status} | حالة الإذن: ${permissionLabel} | Score: ${l.qualification_score||0} | Discovery: ${l.discovery_score||0}</div>${l.source?`<div class="muted">مصدر بيانات العميل: ${l.source}</div>`:''}${l.permission_at?`<div class="muted">تاريخ الإذن: ${l.permission_at}</div>`:''}${l.permission_source?`<div class="muted">مصدر الإذن: ${l.permission_source}</div>`:''}${l.followup_date?`<div class="muted">متابعة: ${l.followup_date}${isFollowupDue(l)?' — مستحقة':''}</div>`:''}${l.last_reply_at?`<div class="muted">آخر رد: ${l.last_reply_at}</div>`:''}${l.notes?`<div>${l.notes}</div>`:''}${l.subject?`<hr><b>${l.subject}</b><pre>${l.body}</pre>`:''}${l.sent_at?`<hr><div class="muted">تم الإرسال: ${l.sent_at} | معرّف الرسالة: ${l.message_id}</div><pre>${l.sent_body||''}</pre>`:''}<div class="grid"><button onclick="qualifyLead('${l.id}')">تأهيل</button><button onclick="makeDraft('${l.id}')">إنشاء مسودة</button><button onclick="recordReply('${l.id}')">تسجيل رد</button><button onclick="editLead('${l.id}')">تفاصيل / تعديل</button><button onclick="setPermission('${l.id}')">حالة الإذن</button><select onchange="setStatus('${l.id}',this.value)">${stages.map(s=>`<option value="${s}" ${s===l.status?'selected':''}>${s}</option>`).join('')}</select><button onclick="removeLead('${l.id}')">حذف</button></div></article>`}
