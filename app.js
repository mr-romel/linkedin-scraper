...
window.add=()=>{
  ...
  if(permission==='NO'&&permissionSource)return l.permission_source=permissionSource;
...
window.sendSelected=async()=>{const ids=[...document.querySelectorAll('.lead-check:checked')].map(x=>x.value);if(!ids.length)return alert('حدد العملاء المحتملين للإرسال');if(ids.length>5)return alert('الحد الأقصى 5 عملاء محتملين في الدفعة');const selected=ids.map(id=>leads.find(l=>l.id===id)).filter(Boolean);const bad=selected.filter(l=>!canSend(l));if(bad.length)return alert('كل عميل محتمل يجب أن يحتوي بريدًا + إذن بالتواصل = لديه موافقة + مسودة جاهزة للإرسال + بدون طلب عدم التواصل ولم يسبق إرساله');if(!getSendEndpoint())return alert('أعد إعداد رابط الإرسال أولًا');if(!confirm(`تأكيد إرسال ${selected.length} رسائل؟`))return;let ok=0,errors=[];for(const original of selected){const i=leads.findIndex(x=>x.id===original.id);try{leads[i]=beginSend(leads[i]);persist();const data=await sendLead(leads[i]);leads[i]=markSent(leads[i],data.message_id);ok++}catch(e){leads[i]={...leads[i],send_lock:false};errors.push(`${original.company}: ${e.message}`)}}persist();render();alert(`تم الإرسال: ${ok}/${selected.length}${errors.length?'\n\nأخطاء:\n'+errors.join('\n'):''}`)};
