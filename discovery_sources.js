...
function stripHtml(value=''){return String(value).replace(/<script[\s\S]*?<\/script>/gi,' ').replace(/<style[\s\S]*?<\/style>/gi,' ').replace(/<[^>]+>/g,' ').replace(/&nbsp;/gi,' ').replace(/&amp;/gi,'&').replace(/&#39;/g,"'").replace(/&quot;/gi,'"').replace(/\s+/g,' ').trim()}
function absoluteUrl(href,baseUrl){try{return new URL(href,baseUrl).href}catch(_){return ''}}
export function extractPublicCompanyCandidates(html,{source='business_directory',baseUrl=''}={}){
  ...
  while((m=anchorRe.exec(text))){
    ...
    if(!href||!label||label.length<2||label.length>120)continue;
    if(/^(home|search|login|sign in|sign up|contact|about|next|previous|load more|view profile|read more)$/i.test(label))continue;
    if(/^(https?:\/\/|mailto:|javascript:)/i.test(label))continue;
    const key=`${label.toLowerCase()}|${href.toLowerCase()}`;if(seen.has(key))continue;seen.add(key);
    candidates.push({company:label,source,source_url:href,discovery_source_url:baseUrl,country:'Egypt'});
  }
  return candidates;
}
export async function discoverFromPublicSource(sourceId,{fetchImpl=globalThis.fetch,timeoutMs=15000}={}){
  ...
  const response=await fetchImpl(source.url,discoveryFetchOptions({timeoutMs}));
  if(!response.ok)throw Error(`فشل جلب المصدر ${source.name}: HTTP ${response.status}`);
  const html=await response.text();
  return extractPublicCompanyCandidates(html,{source:source.source,baseUrl:source.url}).map(x=>({...x,source_id:source.id,source_name:source.name}));
}
